...
excel_file = pd.read_excel (r'C:\Projects\ShareCalculator\Opwek_Maart.xlsx',sheet_name = 1,header = 0,parse_cols = "A,C") 
excel_file.columns = ["Date","Total"]

# define empty cell
empty = ' '
# Find first empty row in timeseries to determine enddate
first_empty_row = excel_file.index[excel_file.iloc[:,0] == empty][0] 
...
###
test_database = pd.read_excel(r'C:\Projects\ShareCalculator\TestDatabase.xlsx',sheet_name = 0,header = 0,parse_cols = "A,B") 
...
writer = pd.ExcelWriter(r'C:\Projects\ShareCalculator\TestDatabase.xlsx', engine = 'xlsxwriter')
